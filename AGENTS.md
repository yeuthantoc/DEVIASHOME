...
Context for AI coding agents (Antigravity reads this file automatically at the project root).
...
1. **Trilingual (EN / KO / VI).** Every user-facing string lives in the `I18N` object in
   `index.html` with keys for `en`, `ko`, `vi`, and is rendered via a `data-i18n="key"`
   attribute. When you add or change copy, you MUST update all three languages and wire the
   element with `data-i18n`. Never hardcode visible text. English is the default.
2. **Analytics must keep working.** GA4 + Meta Pixel snippets are in `<head>`. The `trackLead(source)`
   function fires `generate_lead` (GA4) and `Lead` (Pixel). All CTA links use `data-cta="..."`
   and the lead form calls `trackLead('lead_form')` on submit. Do not remove these.
3. **Colors only from the design tokens** (CSS variables in `:root`). Do not introduce new
   brand colors.
...
## Page sections (top → bottom in `index.html`)
Header (transparent → solid on scroll) · Hero + trust bar · Products (4 categories) ·
...
- **Domain**: `www.your-domain.com` (in canonical, og, hreflang, JSON-LD)
...
- **Korean retailer logos**: `.retailers` (currently plain text — only use official logos if authorized)
- **Form endpoint**: the `// TODO: send data...` line inside the `leadForm` submit handler
  (intended target: an n8n webhook at automation.jobbim.vn or an email service)
...
- Optional: migrate to Next.js + next-intl for per-language URLs (/en /ko /vi) and stronger
  multilingual SEO — only on explicit request. Preserve the exact current design.
